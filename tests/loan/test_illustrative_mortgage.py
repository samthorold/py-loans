...
def test_illustrative_mortgage() -> None:
    ...
    loan_periods = IllustrativeMortgage(
    ...
    assert round(sum(lp.payment for lp in loan_periods.periods)) == 529653
...
def test_illustrative_mortgage_3_terms() -> None:
    ...
    assert (
        abs(round(sum(lp.payment for lp in loan_periods.periods)) - (542939 - 1139))
        <= 1
    ), round(sum(lp.payment for lp in loan_periods.periods))
